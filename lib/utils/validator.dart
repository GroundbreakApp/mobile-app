...
enum StringType { none, alphabetic, numeric, alphaNumeric }

class Validator {
  static int _index = 0;
  static const String _atomCharacters = "!#\$%&'*+-/=?^_`{|}~";
  static StringType _domainType = StringType.none;
  ...
  static String? validateNumber(
    ...
    String? message;
    if (!allowEmpty) message = validateString(value);
    RegExp regExp = allowComma ? RegExp('[^0-9.,]') : RegExp('[^0-9.]');
    if (message == null && regExp.hasMatch(value!) && value.isEmpty == false) {
      message = 'Invalid Input. Only Number Allowed.';
    }
    return message;
  }
  ...
  static String? validateMinLength(
    ...
    if (value == null || value.length < minLength) {
      message = 'The Value Must be Minimal $minLength Positions Long.';
    }
    return message;
  }

  static String? validateWhiteSpaces(String value) {
    String? message;
    if (value.contains(RegExp('\\s+'))) {
      message = 'No Whitespaces Allowed in This Value.';
    }
    return message;
  }
  ...
  static String? validateDropDown<T>(T value) {
    String? message;
    if (value == null) message = 'Please Select';
    return message;
  }
  ...
  static bool _isDomain(String c, bool allowInternational) {
    if (c.codeUnitAt(0) < 128) {
      if (_isLetter(c) || c == '-') {
        _domainType = StringType.alphabetic;
        return true;
      }

      if (_isDigit(c)) {
        _domainType = StringType.numeric;
        return true;
      }
  ...
  static bool _skipDomain(
    ...
    if (_domainType == StringType.numeric) {
      return false;
    }
  ...
  static bool validate(String? email,
    ...
    if (email == null) throw ArgumentError('email');
  ...
  static bool validatePhoneNumber(String string) {
    // Null or empty string is invalid phone number
    if (string.isEmpty) {
      return false;
    }
